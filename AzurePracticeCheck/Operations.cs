...
namespace AzurePracticeCheck
{
    ...
    {
        public static string sqlDataSource = "Server=(localdb)\\MSSQLLocalDB;Database=MovieDB;Trusted_Connection=True;MultipleActiveResultSets=true;";
        ...
        public static void InsertMov(MovieList movie)
        {
            var list = new List<string> { "Science Fiction", "Superhero", "Romance", "Comedy", "Adventure", "Thriller" };
            using (SqlConnection con = new SqlConnection(sqlDataSource))
            {
                //SqlCommand cmd = new SqlCommand("Update MovieList set Id=@Id,Title=@Title,BoxOffice=@BoxOffice,Active=@Active,DateOfLaunch=@DateOfLaunch,HasTeaser=@HasTeaser,GenreId=@GenreId,GenreType=@GenreType where Id=@Id", con);
